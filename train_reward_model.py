...
def format_reward_example(item: Dict) -> str:
    context = item.get("context", "")
    history = item.get("history", [])
    history_text = "\n".join(f"{turn['role']}: {turn['content']}" for turn in history)
    return (
        "You are scoring a player/NPC turn in the Love at Dusk Rowan scene.\n"
        f"Context: {context}\n"
        f"History:\n{history_text}\n"
        f"Player: {item['player_message']}\n"
        f"Rowan: {item['npc_reply']}\n"
        "Return a scalar quality score from 0.0 to 1.0 for slow-burn intimacy, specificity, "
        "boundary respect, and in-character Rowan behavior."
    )
...
def collate_batch(batch, pad_token_id: int):
    ...
    for example in batch:
        ...
        attention_mask.append(torch.nn.functional.pad(example["attention_mask"], (0, pad_length), value=0))
...
def parse_args():
    ...
    parser.add_argument("--train-file", default="datasets/rowan_ashford_reward_all.jsonl")
